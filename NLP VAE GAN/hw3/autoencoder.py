...
class EncoderCNN(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # ====== YOUR CODE: ======
        modules.append(nn.Conv2d(in_channels, 32,7,padding=1))
        modules.append(nn.Conv2d(32,64,7,padding=1))
        modules.append(nn.Dropout2d(0.2))
        modules.append(nn.ReLU())
        modules.append(nn.Conv2d(64,128,5, padding=1))
        modules.append(nn.Conv2d(128,256,5,padding=1))
        modules.append(nn.Dropout2d())
        modules.append(nn.ReLU())
        modules.append(nn.Conv2d(256,out_channels, 3,padding=1))
        modules.append(nn.ReLU())
        # ========================
...
class DecoderCNN(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # ====== YOUR CODE: ======
        modules.append(nn.ConvTranspose2d(in_channels,256,3,padding=1))
        modules.append(nn.ReLU())
        modules.append(nn.ConvTranspose2d(256,128,5,padding=1))
        modules.append(nn.ReLU())
        modules.append(nn.ConvTranspose2d(128,64, 5,padding=1))
        modules.append(nn.ReLU())
        modules.append(nn.ConvTranspose2d(64,32,7,padding=1))
        modules.append(nn.ReLU())
        modules.append(nn.ConvTranspose2d(32,out_channels,7,padding=1))
        # ========================
        self.cnn = nn.Sequential(*modules)
...
class VAE(nn.Module):
    def __init__(self, features_encoder, features_decoder, in_size, z_dim):
        ...
        # ====== YOUR CODE: ======
        self.mean_extractor = nn.Linear(n_features, self.z_dim)
        self.log_variance_extractor = nn.Linear(n_features, self.z_dim)
    ...
    def encode(self, x):
        ...
        mu = self.mean_extractor(h)
        log_sigma2 = self.log_variance_extractor(h)
        u = torch.normal(torch.zeros_like(mu), torch.ones_like(log_sigma2))
