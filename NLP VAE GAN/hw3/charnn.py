...
def chars_to_onehot(text: str, char_to_idx: dict) -> Tensor:
    ...
    # ====== YOUR CODE: ======
    code = [char_to_idx[c] for c in text]
    result = torch.nn.functional.one_hot(torch.tensor(code), len(char_to_idx))
...
def onehot_to_chars(embedded_text: Tensor, idx_to_char: dict) -> str:
    ...
    # ====== YOUR CODE: ======
    idx_list = torch.argmax(embedded_text, dim=1).tolist()
...
def chars_to_labelled_samples(text: str, char_to_idx: dict, seq_len: int, device="cpu"):
    ...
    # ====== YOUR CODE: ======
    embeeded_text = chars_to_onehot(text, char_to_idx)

    S = seq_len
    N = (len(text) - 1) // seq_len

    samples = embeeded_text[0: S * N].reshape(N,S,len(char_to_idx))
    labels = torch.argmax(embeeded_text[1:S*N + 1],dim = 1).reshape(N,S)
...
def hot_softmax(y, dim=0, temperature=1.0):
    ...
    # ====== YOUR CODE: ======
    result = torch.softmax(y/temperature,dim=0)
    # ========================
    return result
...
def generate_from_model(model, start_sequence, n_chars, char_maps, T):
    ...
    with torch.no_grad():
        for i in range(n_chars - len(start_sequence)):
            y, h = model(input_tensor, h)
            prob = hot_softmax(y[0,-1,:], temperature=T)
            idx = torch.multinomial(prob,1)
            out_text += idx_to_char[idx.item()]
            one_hot = chars_to_onehot(out_text[-1], char_to_idx)
            input_tensor = one_hot.unsqueeze(0)
...
class SequenceBatchSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        idx = torch.arange(0,size*self.batch_size).reshape(self.batch_size, size).T.flatten().tolist()
        # ========================
        return iter(idx)
...
class MultilayerGRU(nn.Module):
    ...
    def __init__(self, in_dim, h_dim, out_dim, n_layers, dropout=0):
        ...
        for layer in range(n_layers):
            ...
            current_layer_params[f'xz_{layer}'] = nn.Linear(in_dim, h_dim)
            current_layer_params[f'hz_{layer}'] = nn.Linear(h_dim, h_dim, bias=False)
            current_layer_params[f'xr_{layer}'] = nn.Linear(in_dim, h_dim)
            current_layer_params[f'hr_{layer}'] = nn.Linear(h_dim, h_dim, bias=False)
            current_layer_params[f'xg_{layer}'] = nn.Linear(in_dim, h_dim)
            current_layer_params[f'hg_{layer}'] = nn.Linear(h_dim, h_dim, bias=False)

            for name, module in current_layer_params.items():
    ...
    def forward(self, input: Tensor, hidden_state: Tensor = None):
        ...
        for layer in range(self.n_layers):
            h = layer_states[layer].to(device = input.device)

            next_input = []
            for t in range(seq_len):
                x = layer_input[:,t,:]
                x = x.to(torch.float32)
                W_xz = self.__getattr__(f'xz_{layer}')
                W_hz = self.__getattr__(f'hz_{layer}')
                W_xr = self.__getattr__(f'xr_{layer}')
                W_hr = self.__getattr__(f'hr_{layer}')
                W_xg = self.__getattr__(f'xg_{layer}')
                W_hg = self.__getattr__(f'hg_{layer}')
                ...
                h = z * h + (1 - z)*g
                next_input.append(h)
            layer_input = dropout_layer(torch.stack(next_input,dim=1))
            layer_states[layer] = h

        layer_output = out.forward(layer_input)
