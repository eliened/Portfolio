...
class LeakyReLU(Layer):
    ...
    def backward(self, dout: torch.Tensor):
        ...
        dx = torch.where(x < 0, alpha_tensor, one_tensor) * dout
...
class Linear(Layer):
    ...
    def __init__(self, in_features, out_features, wstd=0.1):
        ...
        self.b = torch.zeros(out_features) * wstd
        # ========================
...
class CrossEntropyLoss(Layer):
    ...
    def backward(self, dout=1.0):
        ...
        # ====== YOUR CODE: ======
        right = x.exp() / torch.sum(x.exp(), dim=1).unsqueeze(-1)
        left = torch.zeros(x.shape).scatter_(-1, y.long().unsqueeze(-1), 1)
        dx = (-left + right) / N
        # ========================
...
class Dropout(Layer):
    ...
    def forward(self, x, **kw):
        ...
        if self.training_mode:
            mat = torch.ones_like(x) * (1 - self.p)
            mat_binary = torch.bernoulli(mat) / (self.p)
            out = x * mat_binary
            self.grad_cache['mat_binary'] = mat_binary
        else:
            out = x
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        if self.training_mode:
            dx = dout * self.grad_cache['mat_binary']
        else:
            dx = dout
        # ========================
...
class MLP(Layer):
    ...
    def __init__(
        ...
        **kw,
        ...
        # ====== YOUR CODE: ======
        activations_dict = {'relu': ReLU, 'sigmoid': Sigmoid, 'tanh': TanH}
        in_features_ = in_features
        for h in hidden_features:
            out_features = h
            layers += [Linear(in_features_, out_features), activations_dict[activation]()]
            if dropout > 0:
                layers += [Dropout(dropout)]
            in_features_ = out_features
