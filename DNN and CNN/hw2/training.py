    ...
    def fit(
        ...
        **kw,
        ...
        for epoch in range(num_epochs):
            ...
            best_test_loss = min(epoch_test_loss)

            if early_stopping is not None and actual_num_epochs > 1:
                if best_test_loss >= prev_best_test_loss:
                    epochs_without_improvement += 1
                else:
                    epochs_without_improvement = 0
                if epochs_without_improvement == early_stopping:
                    break
            prev_best_test_loss = best_test_loss
            # ========================
...
class ClassifierTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        num_correct = ((self.model.classify(X) - y) == 0).sum().item()
    ...
    def test_batch(self, batch) -> BatchResult:
        ...
        with torch.no_grad():
            ...
            # ====== YOUR CODE: ======
            out = self.model.forward(X)
            classification = self.model.classify(X)
            loss = self.loss_fn(out, y)
            batch_loss = loss.item()
            num_correct = ((classification - y) == 0).sum().item()
            # ========================
...
class LayerTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # ====== YOUR CODE: ======
        self.optimizer.zero_grad()
        out = self.model.forward(X.flatten(1,-1))
        loss = self.loss_fn(out, y)
        loss_grad = self.loss_fn.backward()
        self.model.backward(loss_grad)
        self.optimizer.step()
        num_correct = ((torch.argmax(out,dim=1) - y) == 0).sum().item()
