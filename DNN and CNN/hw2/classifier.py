...
class ArgMaxClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        # ====== YOUR CODE: ======
        return torch.argmax(y_proba, dim=1).to(torch.int32)
        # ========================
...
class BinaryClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        # ====== YOUR CODE: ======
        prob = torch.zeros(2)
        prob[self.positive_class] = float(self.threshold)
        prob[1 - self.positive_class] = 1.0 - float(self.threshold)
        tensor = y_proba - prob




        return torch.argmax(tensor, dim = 1).to(torch.int32)
        # ========================
...
def plot_decision_boundary_2d(
    ...
    # ====== YOUR CODE: ======
    M = max(max(x[...,0]), max(x[...,1]))
    m = min(min(x[...,0]), min(x[...,1]))
    xs = torch.arange(m,M,dx)
    ys = torch.arange(m,M,dx)
    x1_grid,x2_grid = torch.meshgrid(xs,ys)
    y_hat = torch.zeros_like(x1_grid)
    for ii,i in enumerate(xs):
        for jj,j in enumerate(ys):
            sample = torch.tensor([[i, j]])
            y_hat[ii][jj] = classifier.classify(sample)[0]
...
def select_roc_thresh(
    ...
    # ====== YOUR CODE: ======
    out = classifier.predict_proba(x).detach()
    #scores = out.gather(1,y.reshape(-1,1))
    fpr, tpr, thresh = sklearn.metrics.roc_curve(y, out[...,classifier.positive_class])
    optimal_thresh_idx = torch.argmin(torch.sqrt(torch.tensor(fpr)**2 + (torch.tensor(tpr) - 1)**2))
